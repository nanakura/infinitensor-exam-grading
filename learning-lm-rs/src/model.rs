        ...
        for group in 0..n_groups {
            for q_seq in 0..seq_len {
                for k_seq in 0..total_seq_len {
                    ...
                    for d in 0..dqkv {
                        let q_idx = q_seq * (n_kv_h * n_groups * dqkv) + (kv_head * n_groups + group) * dqkv + d;
        ...
        for group in 0..n_groups {
            for q_seq in 0..seq_len {
                for d in 0..dqkv {
                    ...
                    let out_idx = q_seq * (n_kv_h * n_groups * dqkv) + (kv_head * n_groups + group) * dqkv + d;
...
fn mlp(
    ...
    for idx in 0..act_data.len() {
        act_data[idx] = act_data[idx] * (1.0 / (1.0 + E.powf(-act_data[idx])))
        * up_data[idx];
    }
    ...
    for idx in 0..residual_data.len() {
        residual_data[idx] += output_data[idx];
    }
}
